...
def compute_effective_reduced_density_HY(Ppr, Tpr, tol=1e-13, verb=False):
    ...
    while True:
        ...
        if np.isnan(abs_rho_func):
            ...
            rho_new = rho_1 - compute_reduced_rho_HY_function_HY(rho_1, X1, X2, X3, X4, Ppr) / compute_derivative_function_HY(
                rho_1, X2, X3, X4)
...
def compute_Z_Hall_Yarborough(sp_gravity, temperature, pressure,
                              # fluid_type
                              ):
...
########### Dranchuk-Purvis-Robinson Functions #################
...
def compute_effective_reduced_density_DPR(Tpr, Ppr, tol=1e-12, runs=200):
    ...
    while 1:
        ...
        rho_new = rho_1 - compute_rho_function_DPR(Tpr, Ppr, rho_1) / compute_derivative_function_DPR(Tpr, Ppr, rho_1)
        delta = abs(rho_1 - rho_new)
