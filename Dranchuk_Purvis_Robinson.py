...
def compute_effective_reduced_density(Tpr, Ppr, tol=1e-12, runs=200):
    ...
    while i <= runs:
        ...
        rhork1 = rho_1 - compute_rho_function(Tpr, Ppr, rho_1) / compute_derivative_function(Tpr, Ppr, rho_1)
        delta = abs(rho_1 - rhork1)

        if delta < tol:
            return rhork1  # Exit the loop if ideal delta found
...
# Example usage
print(compute_Z_with_DPR(0.854, 610, 2000))
